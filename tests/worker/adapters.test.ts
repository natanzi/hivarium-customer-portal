...
describe('FetchOperatorService', () => {
  it('handles valid upstream responses', async () => {
    const fetchArgs: any[] = [];
    const binding = { fetch: async (url: string, init: any) => { fetchArgs.push({url, init}); return new Response(JSON.stringify({
...
describe('FetchLicenseService', () => {
  it('handles signature download customer ID propagation', async () => {
    const fetchArgs: any[] = [];
    const binding = { fetch: async (u: string, init: any) => { fetchArgs.push({url: u, init}); return new Response('signedblob'); } } as any;
    const ls = new FetchLicenseService({ binding, token: 'lic-token' });
    const res = await ls.downloadLicenseDocument('cust-99', 'lic-1');
    expect(res.ok).toBe(true);
    expect((res as any).value).toBe('signedblob');
    expect(fetchArgs[0].url).toContain('customerId=cust-99');
    expect(fetchArgs[0].url).toContain('/licenses/lic-1/document');
